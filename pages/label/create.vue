...
<template>
    ...
    <div>
        <Row>
            ...
            <Card>
                <Form :label-width="80">
                    <FormItem label="标签标题" :error="articleError">
                        <Input v-model="data.title" @on-blur="handleArticletitleBlur" icon="android-list" />
                    </FormItem>
                  <FormItem label="标签颜色">
                    <ColorPicker v-model="data.color" alpha />
                  </FormItem>
                  <Form-item>
...
export default {
  ...
  async asyncData(context, callback) {
    ...
    if (context.route.query.id) {
      ...
      let req = await axios.get('api/admin/label/show/' + id, {})
      if (req.data.data) {
        callback(null, {
          data: req.data.data
        })
  ...
  methods: {
    async CreateCategory() {
      ...
      let req = ''
      if (data.id) {
        req = await axios.put('api/admin/label/edit/' + data.id, data)
      } else {
        req = await axios.post('api/admin/label/create', data)
      }
      if (req.status === 200) {
