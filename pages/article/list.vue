...
<script>
  ...
  import tableData from '~/components/tables/components/article/table-header.js'
  ...
  export default {
    ...
    methods: {
      async getData() {
        ...
        let req = await axios.get('api/admin/article/list', {})
      ...
      async handleDel(val, index) {
        ...
        let req = await axios.delete('api/admin/article/delete/' + id, {})
        if (req.status === 200) {
      ...
      handleChange(val, index) {
        this.$router.push({path: '/article/create/', query: {id: val[index].id}})
