...
<script>
  ...
  import tableData from '~/components/tables/components/category/table-header.js'
  ...
  export default {
    ...
    methods: {
      ...
      async handleDel(val, index) {
        ...
        let req = await axios.delete('api/admin/category/delete/' + id, {})
        if (req.status === 200) {
      ...
      handleChange(val, index) {
        this.$router.push({path:'/category/create/', query:{id:val[index].id}})
